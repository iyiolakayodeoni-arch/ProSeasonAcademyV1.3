...
async function main() {
  ...
  // ══ T6 · the week completes only after all seven days ══
  for (let day = 3; day <= 5; day++) {
    B.recordBaselineMatch(entry(Date.now()));
    B.sealBaselineDay(day);
  }
  const after5 = await B.loadBaseline('chinedu');
  assert.equal(B.currentBaselineDay(after5), 6, 'match 5 sealed → day 6 (the reflection) is current');
  B.saveBaselineReflection('I keep rushing after conceding', 'I now pause before passing');
  B.sealBaselineDay(6);
  const after6 = await B.loadBaseline('chinedu');
  assert.equal(B.currentBaselineDay(after6), 7, 'day 6 sealed → day 7 (the ambition) is current');
  const d7 = after6.days.find((d) => d.day === 7);
  assert.equal(d7.sealedAt, null, 'day 7 not yet sealed');
  assert.equal(B.dayStatus(after6, 7), 'locked', 'day 7 unlocks 24h after the reflection');
  const refl = after6.days.find((d) => d.day === 6).reflection;
  assert.ok(refl && refl.repeated.includes('rushing'), 'day 6 reflection persisted');
  console.log('PASS 6 · the week flows DAY 5 → reflection → ambition, each gated by 24h');
