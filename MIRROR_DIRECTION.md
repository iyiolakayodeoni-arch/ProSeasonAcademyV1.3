...
| Screen recording | ✅ REAL — native console capture consent → console recording MP4 written to app-private storage; recording begins only when the match is detected (§5). Requires a development build (`npx expo run:android` / EAS) — **not** available in Expo Go | Trim/export to the gallery; longer retention settings |
...
| `src/data/manual console review.ts` | Real native contract: recording state, checkpoints, goal-triggered auto-record, stop→path |
| `src/data/baselineScan.ts` | **Baseline Week** — 7-day schedule (30 minutes honesty gap), per-moment analysis model, day content, migration |
...
`npm run typecheck` — clean. `npm test` — 18/18 (7 manual console review + 5 mirror-session + 6 baseline-week). Plugin injection verified with a prebuild harness (`compileModsAsync`).

## 5. The native recording module (added 01 Aug 2026)

`plugins/withmanual console review.js` now injects a **real** implementation at `expo prebuild`
into the generated `android/` project (same package as `MainApplication`, so no import
is needed):

  `console captureManager.createScreenCaptureIntent()` from the current Activity
...
- **Two virtual displays off one projection:**
  ...
  - `mw-record` — full-resolution `console recording` (H.264 MP4, 24fps, 8 Mbps) that
