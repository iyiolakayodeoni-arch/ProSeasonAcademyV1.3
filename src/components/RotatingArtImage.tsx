 ...
 * rendered exactly like a normal Image.
 */
export default function RotatingArtImage({
  ...
  if (choices.length < 2 || index === 0) return <Image source={current} style={style} resizeMode={resizeMode} />;

  return (
    <>
      <Image source={previous} style={style} resizeMode={resizeMode} />
      <Animated.Image key={`art-${index}`} entering={FadeIn.duration(900)} source={current} style={style} resizeMode={resizeMode} />
    </>
  );
}
